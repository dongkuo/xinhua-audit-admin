...
function reset(obj: any, defaultValue: any) {
  for (const key in obj) {
    const value = obj[key]
    if (Array.isArray(value)) {
      obj[key] = defaultValue && defaultValue.hasOwnProperty(key) ? defaultValue[key] : []
    } else if (typeof value === 'object') {
      reset(value, defaultValue)
    } else {
      obj[key] = defaultValue && defaultValue.hasOwnProperty(key) ? defaultValue[key] : null
    }
  }
}
